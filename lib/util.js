const Utilities = {
    ...
    downsampleBuffer(buffer, downsampleRate, mapSample) {
        let bufferLength = buffer.length, downsampledBuffer = new Uint8ClampedArray(bufferLength / downsampleRate), i = 0;
    ...
    eachDownsample(buffer, downSampleRate, fn) {
        let i = 0, bufferLength = buffer.length, downSampledBufferLength = bufferLength / downSampleRate, result = [];
        while (i < bufferLength) {
            var sample = buffer[i];
            if (fn)
    ...
    peakFilter(energies, sensitivity) {
        ...
        let peak = energies[0], secondPeak = energies[1], thirdPeak = energies[2], trough = energies.reverse()[0];
    ...
    doublePeakFilter(energies1, energies2, sensitivity) {
        return (this.peakFilter(energies1, sensitivity) ||
            this.peakFilter(energies2, sensitivity));
    },
    // useful for testing purposes
    generateSineBuffer(frequencies, sampleRate, numberOfSamples, phase = 0) {
        let buffer = new Float32Array(numberOfSamples), volumePerSine = 1 / frequencies.length, i = 0;
    ...
    generateWhiteNoiseBuffer(sampleRate, numberOfSamples) {
        let buffer = new Float32Array(numberOfSamples), i = 0;
        while (i < numberOfSamples) {
    ...
    floatBufferToInt(floatBuffer) {
        let floatBufferLength = floatBuffer.length, intBuffer = new Uint8ClampedArray(floatBufferLength), i = 0;
    ...
    averageDecibels(buffer) {
        ...
        let sum = 0, bufferLength = buffer.length, i = 0;
        while (i < bufferLength) {
